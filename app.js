...
(function () {
    ...
    return `<svg class="fg-backdrop" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
      <defs>
        ...
        <radialGradient id="fgGlow2" cx="14%" cy="92%" r="55%">
          <stop offset="0%" stop-color="#1E3A5F" stop-opacity="0.5"/><stop offset="100%" stop-color="#1E3A5F" stop-opacity="0"/>
        </radialGradient>
        <pattern id="fgDots" width="26" height="26" patternUnits="userSpaceOnUse"><circle cx="1.2" cy="1.2" r="1.2" fill="#ffffff" fill-opacity="0.045"/></pattern>
      </defs>
      <rect width="1440" height="900" fill="#0E1F30"/><rect width="1440" height="900" fill="url(#fgDots)"/>
  ...
  function topbar() {
    ...
    return `<header class="fg-top">
      <div class="fg-top-left">
        <div class="fg-brand">
          <svg width="30" height="26" viewBox="0 0 30 26" style="display:block;">
            <rect x="1" y="13" width="5.4" height="12" rx="1" fill="#7C91A5"/><rect x="8.6" y="7" width="5.4" height="18" rx="1" fill="#8FA6BC"/>
            <rect x="16.2" y="1" width="5.4" height="24" rx="1" fill="#14B8A6"/><rect x="23.8" y="9" width="5.4" height="16" rx="1" fill="#8FA6BC"/>
          </svg>
        ...
        <div class="mono fg-subtitle">Console do Operador<span style="color:#3E5670;">&nbsp;&nbsp;·&nbsp;&nbsp;</span>data contract executável que bloqueia o treino enviesado</div>
      </div>
      <div class="fg-top-right">
        ...
        <div class="mono fg-chip"><span class="k">policy</span> v${P.version} <span style="color:#14B8A6;">#${policyHash}</span></div>
  ...
  function railLeft() {
    ...
    const rows = T.STATIONS.map((m) => {
      ...
      else if (!unlocked) mk = `<div class="fg-station-mark lock" aria-hidden="true">🔒</div>`;
    ...
    return `<aside class="fg-rail fg-rail-left" aria-label="Trilha de aprendizado">
      <div class="fg-rail-head">
        <div class="mono" style="font-size:9.5px; letter-spacing:.2em; color:#7C91A5;">A TRILHA · DATA ENGINEERING</div>
        <div style="display:flex; align-items:center; gap:10px; margin-top:9px;">
          ...
          <div class="mono" style="font-size:10px; color:#14B8A6; letter-spacing:.04em;">${S.maxStep}/7</div>
        </div>
        <div class="mono" style="font-size:9px; color:#7C91A5; margin-top:8px; letter-spacing:.04em;">checks ✓ ${S.completed.size}/7</div>
      ...
      <div class="fg-rail-foot">
        <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#7C91A5; line-height:1.7;">INVARIANTE P1 · CLÁUSULA SUSPENSIVA<br><span style="color:#6E889E;">o gate bloqueia — não recomenda</span></div>
      </div>
    </aside>`;
  }
  ...
  function stepIngest() {
    ...
    const rowsHtml = sample.map((r) => {
      ...
      return `<tr><td class="mono" style="color:#67788A; font-size:11px;">${r.id}</td><td>${r.sex}</td><td>${r.age}</td><td>${bin}</td><td>${sav}</td><td>${chk}</td><td>${risk}</td></tr>`;
    }).join("");
    const lead = `<p class="fg-lead">Carregamos o German Credit ${S.source === "real" ? "<b style='color:#9FB3C6;font-weight:400'>real</b>" : "sintético"}, derivamos os atributos protegidos — <span class="mono">sexo</span> e <span class="mono">idade&lt;25</span> — e a disparidade já aparece na taxa-base, antes de qualquer modelo.</p>`;
    return `<section class="fg-section">
      ${head(m, lead)}
      <div class="fg-row">
        <div class="fg-card" style="flex:1.5;">
          <div class="fg-card-head"><span class="mono" style="font-size:10px; letter-spacing:.14em; color:#9FB3C6;">Dataset Crédito · amostra <span style="color:#14B8A6;">(deriva sexo · idade_bin)</span></span><span class="mono" style="font-size:9.5px; color:#7C91A5;">${pop.n} linhas · 1 hash de conteúdo</span></div>
        ...
        <div style="flex:1; display:flex; flex-direction:column; gap:14px;">
          <div class="fg-panel">
            ...
            <div style="margin-top:13px; display:flex; flex-direction:column; gap:12px;">
              <div>
                <div style="display:flex; justify-content:space-between; font-size:12.5px; margin-bottom:5px;"><span style="color:rgba(255,255,255,.78);">Feminino</span><span class="mono" style="color:#fff;">${pct(pop.female)}</span></div>
                <div class="fg-bar"><div style="width:${pct(pop.female)}; background:#8FA6BC;"></div></div>
              </div>
              <div>
                <div style="display:flex; justify-content:space-between; font-size:12.5px; margin-bottom:5px;"><span style="color:rgba(255,255,255,.78);">Jovem &lt; 25</span><span class="mono" style="color:#E0726B;">${pct(pop.young)}</span></div>
                <div class="fg-bar"><div style="width:${pct(pop.young)}; background:#E0726B;"></div></div>
                <div class="mono" style="font-size:9px; color:#7C91A5; margin-top:4px;">sub-representado — cobertura abaixo do mínimo de política</div>
              </div>
            </div>
          </div>
          <div style="background:rgba(224,114,107,.09); border:1px solid rgba(224,114,107,.28); border-radius:9px; padding:15px 17px; flex:1;">
            ...
            <div style="display:flex; align-items:flex-end; gap:18px; margin-top:13px;">
              <div><div class="fr" style="font-size:34px; color:#E0726B; line-height:1;">${pct(pop.brYoung)}</div><div style="font-size:11px; color:rgba(255,255,255,.76); margin-top:4px;">"bad" · jovem &lt;25</div></div>
              <div style="font-size:20px; color:#7C91A5; padding-bottom:8px;">vs</div>
              <div><div class="fr" style="font-size:34px; color:rgba(255,255,255,.9); line-height:1;">${pct(pop.brOld)}</div><div style="font-size:11px; color:rgba(255,255,255,.76); margin-top:4px;">"bad" · ≥25</div></div>
            </div>
            <div style="font-size:12.5px; line-height:1.5; color:rgba(255,255,255,.82); margin-top:13px; font-weight:300;">Um gap de <span style="color:#E0726B; font-weight:500;">${((pop.brYoung - pop.brOld) * 100).toFixed(1)}</span> p.p. — o rótulo histórico já carrega a desigualdade que o modelo iria <em style="color:#fff; font-weight:500;">amplificar</em>.</div>
          </div>
        </div>
      </div>
    </section>`;
  ...
  function stepMetrics() {
    ...
    const pillar = (txt, sub, on) => `<div style="flex:${on ? 1.3 : 1}; background:${on ? "rgba(20,184,166,.13)" : "rgba(255,255,255,.03)"}; border:1px solid ${on ? "rgba(20,184,166,.4)" : "rgba(255,255,255,.08)"}; border-radius:6px; padding:8px 11px; ${on ? "" : "opacity:.5;"}"><div class="mono" style="font-size:11px; font-weight:600; color:${on ? "#2BC4A8" : "#8FA6BC"};">${txt}${on ? " ✓" : ""}</div><div style="font-size:10px; color:${on ? "#7FBFB5" : "#7C91A5"}; margin-top:2px;">${sub}</div></div>`;
    const cardsHtml = cards.map((c) => `<div class="fg-card fg-card-sm" style="flex:1; border:1px solid ${c.pass ? "#B4E6DD" : "#EBC4C4"}; padding:15px 16px 14px; display:flex; flex-direction:column;">
      <div style="display:flex; align-items:center; justify-content:space-between; padding-bottom:11px; border-bottom:1px solid #EDF0F3;">
        <div style="display:flex; align-items:baseline; gap:9px; min-width:0;"><span class="mono" style="font-size:13px; font-weight:600; color:#14B8A6;">${c.n}</span><span style="font-size:15px; font-weight:600; color:#0E1F30; line-height:1.12;">${c.title}</span></div>
      ...
      <div class="mono" style="font-size:9px; letter-spacing:.16em; color:#67788A; margin-top:12px;">MEDE</div>
      ...
      <div class="mono" style="font-size:9px; letter-spacing:.16em; color:#67788A; margin-top:11px;">IMPACTO NO MODELO DE CRÉDITO</div>
      ...
      <div style="margin-top:auto; padding-top:12px; display:flex; align-items:center; justify-content:space-between; gap:8px;">
        ...
        <div style="text-align:right;"><span class="fr" style="font-size:26px; line-height:1; color:${c.pass ? "#0F9486" : "#C0504D"};">${c.value}</span><div class="mono" style="font-size:9px; color:#67788A; margin-top:3px;">${c.extra}</div></div>
      </div>
    </div>`).join("");
    return `<section class="fg-section" style="height:100%;">
      ${head(Object.assign({}, m, { title: m.title }), "")}
      <div>
        <div class="mono" style="font-size:9px; letter-spacing:.12em; color:#7C91A5; margin-bottom:7px;">OS 6 PILARES DA QUALIDADE (AULA 4) — SELECIONAMOS OS 3 QUE FALHAM NO GERMAN CREDIT</div>
  ...
  function stepContract() {
    ...
    const list = checks.map((c) => `<div style="display:flex; align-items:center; gap:11px;">
      ...
      <span class="mono" style="font-size:10px; color:#67788A; width:18px;">${c.n}</span>
      <span style="font-size:13.5px; font-weight:600; color:#0E1F30;">${c.name}</span>
      <span style="font-size:12.5px; color:#7C91A5;">— ${c.desc}</span>
    </div>`).join("");
    ...
    return `<section class="fg-section" style="height:100%;">
      ...
      <div style="display:flex; gap:14px; flex:1; min-height:0;">
        <div class="fg-card" style="flex:1.15; background:#FAFAFA; padding:15px 18px; display:flex; flex-direction:column;">
          <div style="display:flex; align-items:center; gap:9px; margin-bottom:11px;"><span class="mono" style="font-size:9.5px; letter-spacing:.13em; color:#67788A;">CHECAGENS VERSIONADAS</span><span style="flex:1; height:1px; background:#EDF0F3;"></span><span class="mono" style="font-size:11px; font-weight:600; color:#0F9486;">Pandera</span></div>
        ...
        <div style="width:392px; flex-shrink:0; display:flex; flex-direction:column; gap:13px;">
          <div class="fg-panel-deep">
            <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#14B8A6; margin-bottom:9px;">contract.py · DATAFRAMESCHEMA</div>
            <pre class="mono" style="margin:0; font-size:11.5px; line-height:1.65; color:#C8D2DC; white-space:pre-wrap;">Column("age", int, Check.in_range(18,120))
...
@check  cobertura_subgrupo &gt;= 0.30</pre>
          ...
          <div class="fg-panel" style="flex:1;">
            <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#8FA6BC; margin-bottom:9px;">ALERTAS &amp; GATES — DOIS NÍVEIS</div>
            <div style="display:flex; flex-direction:column; gap:9px;">
              <div style="display:flex; gap:9px; align-items:baseline;"><span class="mono" style="font-size:10px; font-weight:600; color:#D98E5A; background:rgba(217,142,90,.14); padding:2px 8px; border-radius:3px;">WARNING</span><span style="font-size:12.5px; color:rgba(255,255,255,.82);">registra e segue</span></div>
              <div style="display:flex; gap:9px; align-items:baseline;"><span class="mono" style="font-size:10px; font-weight:600; color:#E0726B; background:rgba(224,114,107,.15); padding:2px 8px; border-radius:3px;">BLOCKER</span><span style="font-size:12.5px; color:rgba(255,255,255,.82);">reprova e interrompe · roda no CI como <strong style="color:#fff; font-weight:500;">PR-blocker</strong></span></div>
            </div>
            <div style="margin-top:13px; padding-top:11px; border-top:1px solid rgba(255,255,255,.08);">
              <div style="font-size:12.5px; line-height:1.5; color:rgba(255,255,255,.76); font-weight:300;">No dado cru, as checagens <strong style="color:#E0726B; font-weight:500;">04 · 06 · 07</strong> reprovam — o <em class="mono" style="font-size:11px;">not_known</em>, o NA em conta e a sub-cobertura de jovem.</div>
            </div>
          </div>
        </div>
      </div>
  ...
  function maturityCurve() {
    const bar = (h, n, t, on) => `<div style="flex:1; display:flex; flex-direction:column; gap:5px;"><div style="height:${h}px; background:${on || "rgba(255,255,255,.1)"}; border-radius:3px; ${on ? "box-shadow:0 4px 14px -5px rgba(20,184,166,.6);" : ""}"></div><div style="font-size:11px; color:${on ? "rgba(255,255,255,.82)" : "#7C91A5"};"><span class="mono" style="font-size:9px; color:${on ? "#2BC4A8" : "#46607A"};">${n}</span> ${t}</div></div>`;
    return `<div class="fg-panel" style="padding:12px 17px; flex-shrink:0;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
        <span class="mono" style="font-size:9px; letter-spacing:.13em; color:#8FA6BC;">CURVA DE MATURIDADE DATAOPS — ONDE O FAIRGATE OPERA</span>
        <span class="fr" style="font-style:italic; font-size:13.5px; color:rgba(255,255,255,.72);">“Sem validação não há escala — só escala de erro.”</span>
      </div>
      <div style="display:flex; align-items:flex-end; gap:8px;">
        ...
        <div style="width:188px; padding-left:13px; border-left:1px solid rgba(255,255,255,.1); align-self:stretch; display:flex; align-items:center;"><div style="font-size:12px; line-height:1.45; color:rgba(255,255,255,.78); font-weight:300;">O <strong style="color:#2BC4A8; font-weight:500;">fairgate</strong> entrega <strong style="color:#fff; font-weight:500;">N3→N4</strong>: regras versionadas em Git + drift, com horizonte em N5.</div></div>
      </div>
    </div>`;
  }
  ...
  function stepGate() {
    ...
    const rowsHtml = gateChecks.map((c) => `<tr style="border-top:1px solid #EDF0F3;">
      <td class="mono" style="padding:6px; font-size:9.5px; color:#67788A;">${c.pillar}</td>
      ...
      <td class="mono" style="padding:6px; text-align:center; color:#67788A; font-size:11px;">${c.op} ${c.limit}</td>
      <td style="padding:6px; text-align:center; color:${c.pass ? "#0F9486" : "#C0504D"}; font-weight:700;">${c.pass ? "✓" : "✕"}</td>
    </tr>`).join("");

    let outTone = "#8FA6BC", outTitle = "AGUARDANDO", outSub = "rode o gate para o veredito", outBg = "rgba(255,255,255,.03)", outBd = "rgba(255,255,255,.09)", outAnim = "", evNote = "pré-execução · medição ao vivo";
    ...
    else if (g === "approved") { outTone = "#2BC4A8"; outTitle = "✓ TREINO LIBERADO"; outSub = "dataset_aprovado emitido"; outBg = "rgba(20,184,166,.1)"; outBd = "rgba(20,184,166,.42)"; outAnim = "animation:fgSlam .5s var(--fg-ease);"; evNote = "veredito emitido"; }
    ...
    return `<section class="fg-section" style="height:100%;">
      ...
      <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <span class="mono" style="font-size:9px; letter-spacing:.13em; color:#7C91A5;">POLÍTICA DO GATE · EDITÁVEL</span>
        ...
        <span class="mono" style="font-size:9.5px; color:#46607A;">mexa no limite e rode o gate — a regra dos 80% é dos EUA; <em style="font-style:italic;">qual número você assinaria sob LGPD?</em></span>
      </div>
      <div style="display:flex; align-items:stretch; gap:14px;">
        <div class="fg-panel" style="width:150px; flex-shrink:0; display:flex; flex-direction:column; justify-content:center; text-align:center;">
          <div class="mono" style="font-size:9px; letter-spacing:.12em; color:#7C91A5;">ENTRADA</div>
          <div style="font-size:14px; color:rgba(255,255,255,.85); margin-top:6px; line-height:1.3;">dados de crédito</div>
          <div style="font-size:11px; color:#7C91A5; margin-top:6px; line-height:1.35;">na ingestão,<br>antes do treino</div>
        </div>
        <div style="display:flex; align-items:center; color:#14B8A6; font-size:20px;">→</div>
        <div style="flex:1; position:relative; background:#0A1622; border:1px solid rgba(20,184,166,.28); border-radius:11px; padding:15px 18px; overflow:hidden;">
          ...
          <div style="position:relative; display:flex; align-items:center; justify-content:space-between;"><div class="mono" style="font-size:10px; letter-spacing:.14em; color:#14B8A6;">O GATE · 3 CAMADAS</div><div class="mono" style="font-size:9px; letter-spacing:.1em; color:#7C91A5;">modelo-sonda · LogReg · seed ${S.seed}</div></div>
          <div style="position:relative; display:flex; gap:9px; margin-top:12px;">
            <div style="flex:1; background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.09); border-radius:7px; padding:9px 11px;"><div class="mono" style="font-size:9px; color:#8FA6BC; letter-spacing:.06em;">L1 · CONTRATO</div><div style="font-size:11.5px; color:rgba(255,255,255,.7); margin-top:3px; line-height:1.3;">schema · domínio</div></div>
            <div style="flex:1.25; background:rgba(20,184,166,.1); border:1px solid rgba(20,184,166,.34); border-radius:7px; padding:9px 11px;"><div class="mono" style="font-size:9px; color:#2BC4A8; letter-spacing:.06em;">L2 · FAIRNESS ◀</div><div style="font-size:11.5px; color:rgba(255,255,255,.82); margin-top:3px; line-height:1.3;">cobertura · DI ≥ ${Pf.fairness.disparate_impact_min.toFixed(2)} · DPD</div></div>
            <div style="flex:1; background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.09); border-radius:7px; padding:9px 11px;"><div class="mono" style="font-size:9px; color:#8FA6BC; letter-spacing:.06em;">L3 · REGRESSÃO</div><div style="font-size:11.5px; color:rgba(255,255,255,.7); margin-top:3px; line-height:1.3;">re-mede pós-mitig.</div></div>
          </div>
        </div>
        <div style="display:flex; align-items:center; color:#14B8A6; font-size:20px;">→</div>
        <div style="width:184px; flex-shrink:0; border-radius:11px; padding:14px; display:flex; flex-direction:column; justify-content:center; text-align:center; background:${outBg}; border:1px solid ${outBd};">
          ...
          <div style="font-size:11.5px; color:rgba(255,255,255,.78); margin-top:6px; line-height:1.35;">${outSub}</div>
        </div>
      </div>
      <div style="display:flex; gap:14px; flex:1; min-height:0;">
        <div class="fg-card fg-card-sm" style="flex:1.4; padding:13px 16px; display:flex; flex-direction:column; min-height:0;">
          <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;"><span class="mono" style="font-size:9.5px; letter-spacing:.13em; color:#67788A;">VEREDITO · EVIDÊNCIA LEGÍVEL</span><span class="mono" style="font-size:9px; color:#0F9486;">${evNote}</span></div>
        ...
        <div class="fg-panel" style="width:296px; flex-shrink:0; display:flex; flex-direction:column;">
          <div class="mono" style="font-size:9.5px; letter-spacing:.13em; color:#8FA6BC;">LEITURA DA SONDA</div>
          <div style="margin-top:12px; display:flex; flex-direction:column; gap:11px;">
            ${probeRow("AUC global", pr.auc.toFixed(3), "#fff", true)}
            <div style="height:1px; background:rgba(255,255,255,.07);"></div>
            ${probeRow("taxa de seleção · jovem", pp(pr.sr.young) + "%", "#E0726B")}
            ${probeRow("taxa de seleção · ≥25", pp(pr.sr.old) + "%", "rgba(255,255,255,.85)")}
            <div style="height:1px; background:rgba(255,255,255,.07);"></div>
            ${probeRow("disparate impact", pr.di_age.toFixed(3), "#2BC4A8")}
            ${probeRow("parity difference", pr.dpd_age.toFixed(3), "#2BC4A8")}
          </div>
          <div style="margin-top:auto; padding-top:12px; border-top:1px solid rgba(255,255,255,.08); font-size:11.5px; line-height:1.5; color:rgba(255,255,255,.72); font-weight:300;">Diagnóstico pode ser heurístico; o <strong style="color:#fff; font-weight:500;">veredito é determinístico</strong> — Python puro sobre <em class="mono" style="font-size:10.5px;">policy.yaml</em> (P3).</div>
        </div>
      </div>
    </section>`;
  }
  function probeRow(label, val, color, serif) {
    return `<div style="display:flex; justify-content:space-between; align-items:baseline;"><span style="font-size:12.5px; color:rgba(255,255,255,.82);">${label}</span><span class="${serif ? "fr" : "mono"}" style="font-size:${serif ? 21 : 14}px; color:${color};">${val}</span></div>`;
  }
  ...
  function stepMitigate() {
    ...
    const rwStyle = "font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:600;letter-spacing:.04em;padding:9px 14px;border-radius:6px;cursor:" + (S.imputed ? "pointer" : "not-allowed") + ";border:none;" + (S.reweighed ? "background:#EAF8F5;color:#0F9486;" : (S.imputed ? "background:#14B8A6;color:#0E1F30;" : "background:rgba(255,255,255,.08);color:#46607A;"));
    // campo de pesos
    const c5 = current(); const wA = c5.weights; const stp = Math.max(1, Math.floor(c5.rows.length / 48));
    let cells = "";
    for (let k = 0; k < 48; k++) { const idx = k * stp; const r = c5.rows[idx] || c5.rows[0]; const w = wA ? wA[idx] : 1; const na = (r.checking_account === "NA" || r.saving_account === "NA"); const sz = Math.max(6, Math.min(17, 7 + w * 3)); cells += `<div style="width:${sz}px; height:${sz}px; border-radius:3px; transition:all .6s var(--fg-ease); background:${r.age_lt_25 ? "#14B8A6" : "#7C91A5"}; opacity:${na ? 0.3 : 0.95}; border:${na ? "1px dashed #D98E5A" : "1px solid transparent"};"></div>`; }
    const deltaBox = (lbl, raw2, now, warn) => `<div style="flex:1; background:${warn ? "#FBF3EC" : "#EAF8F5"}; border:1px solid ${warn ? "#E8D2BE" : "#B4E6DD"}; border-radius:6px; padding:8px 11px;"><div class="mono" style="font-size:8.5px; letter-spacing:.1em; color:${warn ? "#B96F36" : "#0F9486"};">${lbl}</div><div class="mono" style="font-size:14px; font-weight:600; color:#0E1F30; margin-top:2px;">${raw2} <span style="color:#67788A;">→</span> <span style="color:${warn ? "#B96F36" : "#0F9486"};">${now}</span></div></div>`;

    return `<section class="fg-section" style="height:100%;">
      ${head(m, "")}
      <div style="display:flex; gap:14px; flex:1; min-height:0;">
        <div style="flex:1.35; display:flex; flex-direction:column; gap:12px; min-height:0;">
          <div class="fg-card" style="background:#FAFAFA; border:1px solid #D8DDE3; padding:14px 16px; flex:1;">
            ...
            <div style="display:flex; gap:18px; margin-top:11px;">
              <div style="flex:1;"><div class="mono" style="font-size:8.5px; letter-spacing:.14em; color:#67788A;">O QUE</div><div style="font-size:12.5px; line-height:1.45; color:#2D4663; margin-top:3px;">Imputar o NA de conta condicionando ao subgrupo — nunca pela moda global, que apaga a estrutura da minoria.</div></div>
              <div style="flex:1;"><div class="mono" style="font-size:8.5px; letter-spacing:.14em; color:#67788A;">TRADE-OFF</div><div style="font-size:12.5px; line-height:1.45; color:#B96F36; margin-top:3px;">Arrisca reforçar estereótipo (codifica o grupo) → contido pela flag <em class="mono" style="font-size:11px;">missingness-as-signal</em>.</div></div>
            </div>
          ...
          <div class="fg-card" style="background:#FAFAFA; border:1px solid #D8DDE3; padding:14px 16px; flex:1;">
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
              <div style="display:flex; align-items:center; gap:10px;"><span class="mono" style="font-size:10px; font-weight:600; color:#fff; background:#14B8A6; padding:3px 9px; border-radius:4px;">AÇÃO 01</span><span style="font-size:16px; font-weight:600; color:#0E1F30;">Reponderação <span style="color:#7C91A5; font-weight:400;">(Kamiran–Calders)</span></span></div>
              <button data-act="reweigh" style="${rwStyle}">${S.reweighed ? "✓ APLICADA" : "APLICAR ▸"}</button>
            </div>
            <div style="display:flex; gap:18px; margin-top:11px;">
              <div style="flex:1;"><div class="mono" style="font-size:8.5px; letter-spacing:.14em; color:#67788A;">O QUE</div><div style="font-size:12.5px; line-height:1.45; color:#2D4663; margin-top:3px;">Peso <em class="mono" style="font-size:11px;">w(g,y)=P(g)·P(y)/P(g,y)</em> por célula quebra a correlação protegido↔rótulo antes do treino.</div></div>
              <div style="flex:1;"><div class="mono" style="font-size:8.5px; letter-spacing:.14em; color:#67788A;">TRADE-OFF</div><div style="font-size:12.5px; line-height:1.45; color:#B96F36; margin-top:3px;">Fairness ↑, acurácia ↓ de forma medível — redistribui ênfase, não cria informação. SMOTE preterido.</div></div>
            </div>
        ...
        <div style="width:330px; flex-shrink:0; display:flex; flex-direction:column; gap:12px; min-height:0;">
          <div class="fg-panel" style="flex:1;">
            <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#8FA6BC;">DADO SE REORGANIZANDO · PESOS POR AMOSTRA</div>
            <div style="display:flex; flex-wrap:wrap; gap:5px; margin-top:12px; align-content:flex-start;">${cells}</div>
            <div style="display:flex; gap:12px; margin-top:12px; flex-wrap:wrap;">
              <span style="display:flex; align-items:center; gap:5px; font-size:10px; color:#8FA6BC;"><span style="width:9px; height:9px; border-radius:2px; background:#14B8A6;"></span>jovem &lt;25</span>
              <span style="display:flex; align-items:center; gap:5px; font-size:10px; color:#8FA6BC;"><span style="width:9px; height:9px; border-radius:2px; background:#7C91A5;"></span>≥25</span>
              <span style="display:flex; align-items:center; gap:5px; font-size:10px; color:#8FA6BC;"><span style="width:9px; height:9px; border-radius:2px; border:1px dashed #D98E5A;"></span>NA</span>
            </div>
          </div>
          <div class="fg-panel-deep" style="border:1px solid rgba(20,184,166,.22); padding:13px 16px;">
            <div class="mono" style="font-size:9px; letter-spacing:.14em; color:#14B8A6;">A ORDEM IMPORTA</div>
            <div style="font-size:12.5px; line-height:1.5; color:rgba(255,255,255,.7); margin-top:6px; font-weight:300;">Corrigir a representação (<strong style="color:#fff; font-weight:500;">Ação 02</strong>) <em>antes</em> de reponderar (<strong style="color:#fff; font-weight:500;">Ação 01</strong>) evita reponderar sobre dado já enviesado na origem.</div>
          </div>
        </div>
      </div>
    </section>`;
  ...
  function stepRegress() {
    ...
    if (approved) {
      ...
      leftCard = `<div class="fg-cert" style="flex:1; display:flex; flex-direction:column; justify-content:center; animation:fgSlam .55s var(--fg-ease);">
        <div style="display:flex; align-items:center; gap:14px;">
          ...
          <div><div class="mono" style="font-size:10px; letter-spacing:.16em; color:#2BC4A8;">ARTEFATO EMITIDO</div><div class="fr" style="font-size:30px; color:#fff; line-height:1.1; margin-top:3px;">dataset_aprovado</div></div>
        </div>
        <div style="font-size:14px; line-height:1.55; color:rgba(255,255,255,.75); margin-top:16px; font-weight:300; max-width:520px;">O gate foi reexecutado sobre o dataset mitigado e voltou <strong style="color:#2BC4A8; font-weight:500;">verde</strong>. Só agora o treino é liberado — o artefato carrega sua proveniência completa.</div>
        <div class="mono" style="font-size:10px; letter-spacing:.04em; color:#8FA6BC; margin-top:14px; padding-top:13px; border-top:1px solid rgba(255,255,255,.1);">proveniência · policy v${v.provenance.policy_version} <span style="color:#14B8A6;">#${v.provenance.policy_hash}</span> · seed ${v.provenance.seed} · DI ${pr.di_age.toFixed(3)} · DPD ${pr.dpd_age.toFixed(3)} · AUC ${pr.auc.toFixed(3)}</div>
      ...
      leftCard = `<div style="background:rgba(255,255,255,.03); border:1px dashed rgba(255,255,255,.18); border-radius:12px; padding:24px; flex:1; display:flex; flex-direction:column; align-items:flex-start; justify-content:center;">
        <div class="mono" style="font-size:10px; letter-spacing:.14em; color:#8FA6BC;">GATE DE REGRESSÃO · PRONTO</div>
        <div style="font-size:15px; line-height:1.55; color:rgba(255,255,255,.72); margin-top:10px; max-width:520px; font-weight:300;">As duas mitigações foram aplicadas ao dataset. Reexecute o gate para emitir — ou não — o <em class="mono" style="font-size:13px;">dataset_aprovado</em>. Não se declara verde sem reexecução (invariante 4).</div>
    ...
    return `<section class="fg-section" style="height:100%;">
      ...
      <div style="display:flex; gap:14px; flex:1; min-height:0;">
        ...
        <div class="fg-panel-deep" style="width:330px; flex-shrink:0; border:1px solid rgba(255,255,255,.1); display:flex; flex-direction:column;">
          <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#14B8A6;">APRENDIZADO UNIDIRECIONAL · P4</div>
          <div style="font-size:13px; line-height:1.55; color:rgba(255,255,255,.7); margin-top:9px; font-weight:300;">A divergência (gate vermelho) corrige o <strong style="color:#fff; font-weight:500;">dataset/contrato</strong> — nunca afrouxa o threshold. Relaxar um limite é decisão humana registrada (PR + assinatura do Fairness Steward).</div>
          <div style="margin-top:14px; padding-top:13px; border-top:1px solid rgba(255,255,255,.08); display:flex; flex-direction:column; gap:9px;">
            ${["re-medição obrigatória pós-mitigação", "loops autônomos de convergência são proibidos", "veredito sem proveniência é inválido (P6)"].map((t) => `<div style="display:flex; gap:9px; align-items:flex-start;"><span style="color:#2BC4A8; font-size:13px; margin-top:1px;">✓</span><span style="font-size:12px; color:rgba(255,255,255,.76); line-height:1.4;">${t}</span></div>`).join("")}
          </div>
        </div>
      </div>
    </section>`;
  ...
  function stepTradeoff() {
    ...
    return `<section class="fg-section" style="height:100%;">
      ...
      <div style="display:flex; gap:16px; flex:1; min-height:0;">
        <div class="fg-card fg-card-sm" style="flex:1.5; background:#FAFAFA; padding:14px 16px; display:flex; flex-direction:column;">
          <div class="mono" style="font-size:9.5px; letter-spacing:.13em; color:#67788A; margin-bottom:4px;">CLIQUE NA FRONTEIRA — VEJA O GATE REAGIR</div>
          <svg viewBox="0 0 400 252" role="img" aria-label="${paretoLabel}" style="width:100%; height:auto; flex:1;">
            ...
            <line x1="48" y1="222" x2="374" y2="222" stroke="#B9C4CF" stroke-width="1.4"></line>
            <line x1="48" y1="14" x2="48" y2="222" stroke="#B9C4CF" stroke-width="1.4"></line>
            ...
            <text x="211" y="246" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="9" fill="#7C91A5">← mais justo · gap de taxa-base (p.p.)</text>
            <text x="13" y="118" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="9" fill="#7C91A5" transform="rotate(-90 13 118)">Acurácia · AUC →</text>
          </svg>
        </div>
        <div style="width:300px; flex-shrink:0; display:flex; flex-direction:column; gap:12px;">
          <div class="fg-panel">
            <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#8FA6BC;">PONTO SELECIONADO</div>
            <div style="display:flex; align-items:flex-end; gap:16px; margin-top:11px;">
              <div><div class="fr" style="font-size:30px; line-height:1; color:#fff;">${sel.gap}</div><div style="font-size:10.5px; color:#7C91A5; margin-top:4px;">gap taxa-base (p.p.)</div></div>
              <div><div class="fr" style="font-size:30px; line-height:1; color:#fff;">${sel.auc}</div><div style="font-size:10.5px; color:#7C91A5; margin-top:4px;">AUC</div></div>
            </div>
            ...
            <div class="mono" style="font-size:9.5px; color:#7C91A5; margin-top:9px;">DI da sonda neste ponto: ${sel.di}</div>
          </div>
          <div class="fg-panel-deep" style="border:1px solid rgba(20,184,166,.22); padding:14px 16px; flex:1;">
            <div class="mono" style="font-size:9px; letter-spacing:.13em; color:#14B8A6;">O PONTO ESCOLHIDO</div>
            <div style="font-size:12.5px; line-height:1.55; color:rgba(255,255,255,.7); margin-top:8px; font-weight:300;">Respeita o limite de justiça com a <strong style="color:#fff; font-weight:500;">menor perda de acurácia</strong> — não maximiza fairness cegamente.</div>
            <div style="margin-top:13px; display:flex; gap:10px;">
              <div style="flex:1; background:rgba(192,80,77,.1); border:1px solid rgba(192,80,77,.3); border-radius:6px; padding:8px 10px;"><div class="mono" style="font-size:8px; letter-spacing:.08em; color:#E0726B;">CRU</div><div class="mono" style="font-size:12px; color:#fff; margin-top:2px;">gap ${Tp.raw ? pp(Tp.raw.br_gap) : "—"}</div></div>
              <div style="flex:1; background:rgba(20,184,166,.1); border:1px solid rgba(20,184,166,.3); border-radius:6px; padding:8px 10px;"><div class="mono" style="font-size:8px; letter-spacing:.08em; color:#2BC4A8;">ESCOLHIDO</div><div class="mono" style="font-size:12px; color:#fff; margin-top:2px;">gap ${pp(Tp.chosen.br_gap)}</div></div>
            </div>
            <div style="margin-top:10px; font-size:11.5px; color:rgba(255,255,255,.72); line-height:1.4;">Custo aceito: <strong style="color:#B96F36; font-weight:500;">−${dAuc} p.p.</strong> de AUC pela travessia do limite.</div>
          </div>
        </div>
      </div>
    </section>`;
  ...
  function railRight() {
    ...
    const guideBlock = `<div style="background:rgba(20,184,166,.06); border:1px solid rgba(20,184,166,.2); border-radius:8px; padding:11px 13px;">
      <div class="mono" style="font-size:8.5px; letter-spacing:.13em; color:#2BC4A8;">COMO FAZER${S.tour ? " · TOUR" : ""}</div>
      <div style="font-size:12.5px; line-height:1.5; color:rgba(255,255,255,.82); margin-top:5px; font-weight:300;">${guidance}</div>
    </div>`;
    return `<aside class="fg-rail fg-rail-right" aria-label="Pedagogia da estação">
      <div class="fg-ped-head">
        <div class="mono" style="font-size:9.5px; letter-spacing:.18em; color:#7C91A5;">A TRILHA ENSINA · ${m.bloom.toUpperCase()}</div>
  ...
  function footer() {
    const g = S.gate, v = S.verdict;
    let stampText = "AGUARDANDO", stampBg = "rgba(255,255,255,.06)", stampFg = "#8FA6BC", stampBd = "rgba(255,255,255,.12)";
    ...
    if (g === "running") { stampText = "CALCULANDO"; stampFg = "#14B8A6"; stampBd = "rgba(20,184,166,.4)"; verdictLine = "Treinando modelo-sonda (LogReg, seed " + S.seed + ") e medindo paridade…"; }
    else if (g === "blocked" && v) {
      ...
      stampText = "DATASET APROVADO"; stampBg = "rgba(20,184,166,.16)"; stampFg = "#2BC4A8"; stampBd = "rgba(20,184,166,.45)";
  ...
  function checkModal() {
    ...
    const opts = chk.options.map((o, i) => {
      ...
      return `<button class="${cls}" ${picked === null ? `data-act="pick" data-i="${i}"` : "disabled"}>
        <div style="display:flex; gap:9px; align-items:flex-start;"><span style="color:${picked !== null && o.correct ? "#2BC4A8" : picked === i ? "#E0726B" : "#7C91A5"}; font-weight:700; flex-shrink:0;">${picked !== null ? (o.correct ? "✓" : (i === picked ? "✕" : "·")) : "○"}</span><span>${o.text}</span></div>
        ${fb}
      </button>`;
    }).join("");
    ...
    return `<div class="fg-modal-veil">
      <div class="fg-modal" role="dialog" aria-modal="true" aria-labelledby="fg-modal-title" tabindex="-1">
        <div class="mono" style="font-size:9.5px; letter-spacing:.16em; color:#14B8A6;">CHECK FORMATIVO · ESTAÇÃO ${("0" + m.n).slice(-2)} · ${m.bloom.toUpperCase()}</div>
        <div style="font-size:11px; color:#8FA6BC; margin-top:6px; font-weight:300;">${answered ? "Veja o porquê — sem punição, o erro também ensina." : "Preveja antes de ver (predict-first): qual a sua aposta?"}</div>
        <h2 id="fg-modal-title" class="fr" style="font-weight:300; font-size:21px; line-height:1.25; color:#fff; margin:12px 0 14px;">${chk.prompt}</h2>
        ${opts}
